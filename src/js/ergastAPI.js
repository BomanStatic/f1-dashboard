export class ErgastAPI {
  ...
  async fetchDriver(driverId){
    const resultDriver = await fetch( `${this.baseURL}/drivers/${driverId}.json`);
  ...
  async fetchResults(driverId){
    const resultResults = await fetch(`${this.baseURL}/current/drivers/${driverId}/results.json`);
    if(resultResults.ok){
  ...
  async fetchConstructor(driverId){
    const constructorResults = await fetch(`${this.baseURL}/current/drivers/${driverId}/constructors.json`);
  ...
  async getNextThreeRaces() {
    ...
    if (response.ok) {
      ...
      for (let i = 1; i < 4; i++) {
        ...
        const nextRaceResponse = await fetch(`${this.baseURL}/${currentYear}/${nextRound}.json`);
        if (nextRaceResponse.ok) {
          const nextRaceData = await nextRaceResponse.json();
          const subsequentRace = nextRaceData.MRData.RaceTable.Races[0];
